...
from vectorforge_v1.llm_gateway import require_openai_api_key
...
def _route_autogluon_problem(
    *,
    ...
    # Flattened layout: the designer run lives at runs/<run_id>/designers/<problem_id>/autogluon/.
    # ArtifactStore resolves run dirs as runs_dir/run_id, so we root runs_dir at the
    # problem dir and use the engine name as the run_id → the engine stays a path segment
    # without the redundant run_X_ prefix. The parent path already scopes uniqueness.
    designer_runs_dir = run_dir / "designers" / problem_id
    ...
    # thread_id must be globally unique across orchestrator runs (shared checkpointer),
    # while the on-disk run dir stays flat at designers/<problem_id>/.
    graph_config = {"configurable": {"thread_id": f"{run_id}_{problem_id}_autogluon"}}
...
def _route_autorag_problem(
    *,
    ...
    # AutoRAG runs entirely on the OpenAI API — require a real OpenAI key.
    require_openai_api_key(context="AutoRAG designer")
    ...
    # Flattened layout: designer run dir at runs/<run_id>/designers/<problem_id>/autorag/.
    # work_dir is decoupled from run_id here, so the run_id value stays engine-tagged
    # and globally unique (for manifest/artifact naming) while the dir keeps the engine
    # as a path segment without the redundant run_X_ prefix.
    designer_run_dir = run_dir / "designers" / problem_id / "autorag"
    designer_run_id = f"{run_id}_{problem_id}_autorag"
    ...
    try:
        ...
        (designer_run_dir / "error_traceback.txt").write_text(tb, encoding="utf-8")
        raise
    return {
        "problem_id": problem_id,
        "designer": "autorag",

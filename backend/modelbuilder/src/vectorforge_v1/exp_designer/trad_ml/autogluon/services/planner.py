...
from vectorforge_v1.llm_gateway import model_json_schema, structured_llm_call
...
def get_planner() -> Planner:
    if get_settings().planner_provider in {"ai_gateway", "openai"}:
        return AIGatewayPlanner()
    return MockPlanner()
...
class AIGatewayPlanner:
    def __init__(self, artifact_store: ArtifactStore | None = None) -> None:
        self.artifact_store = artifact_store or ArtifactStore()
        settings = get_settings()
        self.model = settings.ai_gateway_model or settings.openai_model
    ...
    def plan_round(
        ...
        if len(model.experiments) != experiments_per_round:
            raise ValueError(
                f"AI Gateway planner returned {len(model.experiments)} experiments; expected {experiments_per_round}"
            )
        path = self.artifact_store.run_dir(run_id) / "planning" / f"round_{round_number}_plan.json"
        return self.artifact_store.write_json(path, model.model_dump())
    ...
    def _parse(self, schema, messages: list[dict[str, Any]]):
        ...
        if not api_key:
            raise RuntimeError(
                "AI Gateway planner is enabled but no gateway credential is configured. "
        ...
        return structured_llm_call(
            ...
            tool_description=f"Return a valid {schema.__name__} payload.",
            output_schema=model_json_schema(schema),
            api_key=api_key,
            model=self.model,
        )

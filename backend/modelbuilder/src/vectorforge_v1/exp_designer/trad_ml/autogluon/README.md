...
Planner provider can be configured with `VECTORFORGE_PLANNER_PROVIDER`. To use Vercel AI Gateway structured planner output, set the key in environment or `.env`:

```bash
export AI_GATEWAY_API_KEY="your_api_key_here"
VECTORFORGE_PLANNER_PROVIDER=ai_gateway uv run uvicorn vectorforge_v1.exp_designer.trad_ml.autogluon.main:app --reload
```
...
VECTORFORGE_PLANNER_PROVIDER=ai_gateway
AI_GATEWAY_MODEL=anthropic/claude-sonnet-4.6
VECTORFORGE_AUTOGLUON_AI_GATEWAY_MODEL=anthropic/claude-sonnet-4.6
VECTORFORGE_NARRATIVE_AI_GATEWAY_MODEL=anthropic/claude-sonnet-4.6
AUTORAG_AGENT_MODEL=anthropic/claude-sonnet-4.6
AUTORAG_AGENT_GEVAL_MODEL=anthropic/claude-sonnet-4.6
```

`VECTORFORGE_AI_GATEWAY_API_KEY` and `VERCEL_OIDC_TOKEN` are also accepted.

AutoGluon planning uses `VECTORFORGE_AUTOGLUON_AI_GATEWAY_MODEL`, artifact narrative writing uses
`VECTORFORGE_NARRATIVE_AI_GATEWAY_MODEL`, and AutoRAG uses `AUTORAG_AGENT_MODEL` plus
`AUTORAG_AGENT_GEVAL_MODEL`. All default to `anthropic/claude-sonnet-4.6`.

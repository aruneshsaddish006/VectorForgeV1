...
def write_final_recommendation(state: RunState) -> dict:
    ...
    ArtifactStore().write_status(state["run_id"], "completed", {"final_recommendation_path": str(path)})
    return {
        "status": "completed",
        "final_recommendation_path": str(path),
        "events": [{"node": "write_final_recommendation", "status": "completed", "message": "Final recommendation written."}],
    }
